...
const Schedule = () => {
  ...
  const targetDate = new Date("2026-07-28T20:00:00+01:00");
  ...
  useEffect(() => {
    ...
    // 2. Premium GSAP Matrix Animation Entry
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({ defaults: { ease: "power4.out" } });

      // Smooth structural reveal
      tl.fromTo(
        cardRef.current,
      ...
      // Hardware split-flap sequential drop stagger
      tl.fromTo(
        ".ticker-block",
  ...
  return (
    <section
      ref={containerRef}
      className="w-full py-24 px-4 flex flex-col items-center justify-center bg-slate-950 text-white perspective-[1000px]"
    >
      {/* Editorial Intro Typography */}
      <div className="flex flex-col items-center justify-center text-center max-w-xl mx-auto mb-12 space-y-3">
        <h2 className="text-primary-yellow font-display text-3xl font-black tracking-tight sm:text-4xl">
          Ready to Take Charge of Your Life?
        </h2>
        <p className="text-sm text-slate-400 font-sans max-w-sm">
          Your goals are waiting on the other side of your execution.
        </p>
      </div>

      {/* ULTRA PREMIUM DARK BENTO COMPONENT */}
      <div
        ref={cardRef}
        className="w-full max-w-xl bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-[40px] p-6 sm:p-10 shadow-2xl relative overflow-hidden"
      >
        {/* Layered Subtle Neon Background Backglows */}
        <div className="absolute top-0 right-0 w-[200px] h-[200px] bg-indigo-500/10 rounded-full blur-[80px] pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-[200px] h-[200px] bg-rose-500/5 rounded-full blur-[80px] pointer-events-none" />
        ...
        <div className="flex items-center justify-between w-full mb-6 relative z-10">
          ...
          {/* Cyber-Pill Calendar Badge */}
          <div className="relative">
            <div className="pill-glow absolute inset-0 bg-rose-500/20 blur-md rounded-full pointer-events-none" />
            <span className="relative inline-block bg-rose-950/80 border border-rose-500/30 text-rose-400 font-mono font-bold text-xs px-4 py-1.5 rounded-full backdrop-blur-sm shadow-inner">
              28 July
            </span>
          </div>
        </div>
        ...
        {/* Hero Context Phrase */}
        <h3 className="text-4xl sm:text-5xl font-black font-sans tracking-tight mb-10 text-slate-100 relative z-10">
          In {timeLeft.days} days
        </h3>

        {/* HARDWARE DIGITAL CONTAINER DISPLAY */}
        <div className="w-full bg-black/60 rounded-[28px] p-5 sm:p-6 flex flex-col items-center justify-center relative border border-white/5 shadow-[inset_0_2px_8px_rgba(0,0,0,0.8)] z-10">
          <span className="text-[10px] uppercase font-mono tracking-[0.25em] text-slate-600 self-start mb-4 pl-1">
            Live Stream Countdown
          </span>

          {/* Monospace Hardware Grid Core */}
          <div className="flex items-center justify-center gap-1 sm:gap-2 font-mono text-2xl sm:text-4xl font-black text-white select-none">
            {/* Days Column */}
            <div className="ticker-block bg-neutral-900/90 border border-neutral-800 rounded-xl px-3 py-3 shadow-lg text-center min-w-[58px] sm:min-w-[68px]">
              {pad(timeLeft.days)}
            </div>
            <span className="text-neutral-800 animate-pulse font-sans -mt-1 mx-0.5">
              :
            </span>
            ...
            {/* Seconds Column Accent */}
            <div className="ticker-block bg-neutral-900/90 border border-neutral-800/80 rounded-xl px-3 py-3 shadow-lg text-center min-w-[58px] sm:min-w-[68px] text-rose-400 font-bold bg-gradient-to-b from-neutral-900 to-neutral-950">
              {pad(timeLeft.seconds)}
            </div>
          </div>

          {/* Dynamic Grid Alignment Sublabels */}
          <div className="flex justify-center items-center gap-1 sm:gap-2 w-full mt-3 text-[10px] uppercase font-mono tracking-widest text-slate-600 text-center">
